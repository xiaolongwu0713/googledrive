...
################################################################ INIT #################################################################################
	...
	dpath='/Users/long/Documents/BCI/python_scripts/examples/GumbelChannelSelection'
	...
	temperature_schedule = exponential_decay_schedule(args.start_temp,args.end_temp,args.epochs,int(args.epochs*3/4)) # beta: 10-0.1
	thresh_schedule = exponential_decay_schedule(3.0,1.1,args.epochs,args.epochs) # tau: 3-1.1. Duplicate selection panelty
...
################################################################ SUBJECT-INDEPENDENT CHANNEL SELECTION #################################################################################
	...
	while epoch in range(args.epochs) and (not early_stop):
		...
		if((torch.mean(H.data)<=args.entropy_lim) and (args.stop_delta>prev_val_loss-val_loss)):
			patience_timer+=1
			if(args.verbose):
