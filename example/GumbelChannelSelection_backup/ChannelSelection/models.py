...
class MSFBCNN(nn.Module):
	...
	def forward(self, x):
		...
		# Layer 1
		x1 = self.conv1a(x); #([16, 10, 3, 1125])
		x2 = self.conv1b(x); #([16, 10, 3, 1125])
		x3 = self.conv1c(x); #([16, 10, 3, 1125])
		x4 = self.conv1d(x); #([16, 10, 3, 1125])

		# concatenate alone plan, not time
		x = torch.cat([x1,x2,x3,x4],dim=1) #([16, 40, 3, 1125])
		x = self.batchnorm1(x)
...
class SelectionLayer(nn.Module):
	def __init__(self, N,M,temperature=1.0):
		...
		self.N = N
		self.M = M
		self.qz_loga = Parameter(torch.randn(N,M)/100) # alpha in the paper
	...
	def quantile_concrete(self, x):
		g = -torch.log(-torch.log(x)) # Gumble distri
		y = (self.qz_loga+g)/self.temperature
		y = torch.softmax(y,dim=1)
		return y

	def regularization(self): # duplicate selection panelty
		eps = 1e-10
	...
	def sample_z(self, batch_size, training):
		if training:
			...
			return z # Equation 2. z is w in the paper

		else:
			ind = torch.argmax(self.qz_loga,dim=0)
			one_hot = self.floatTensor(np.zeros((self.N,self.M))) # (44,3)
	...
	def regularizer(self,lamba,weight_decay):
		...
		for i,layer in enumerate(self.layers):
			if(type(layer) == SelectionLayer):
				reg_selection += layer.regularization() # duplicate selection panelty
			else:
