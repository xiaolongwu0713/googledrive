...
from grasp.TSception.utils import EEGDataset
...
class TrainModel():
    ...
    def Leave_one_session_out(self):
        ...
        '''
        save_path = '/Users/long/Documents/BCI/python_scripts/grasp/TSceptionEEG'
        ...
        data = self.data # (2, 6, 574, 1, 4, 1024)
        label = self.label # (2, 6, 574)
    ...
    def train(self, train_data, train_label, test_data, test_label, val_data,
        ...
        traindata = traindata.transpose(2,0,1) # (118, 15000, 20)
        valdata = valdata.transpose(2,0,1)
        testdata = testdata.transpose(2,0,1) # (8, 15000, 20)
        trainx, trainy = traindata[:, :-1, :], traindata[:, -1, :]
        valx, valy = valdata[:, :-1, :], valdata[:, -1, :]
        testx, testy = testdata[:, -1, :], testdata[:, -1, :]

        dataset_train = EEGDataset(trainx, trainy)
        dataset_val = EEGDataset(valx, valy)
        dataset_test = EEGDataset(testx, testy)
...
if __name__ == "__main__":
    train = TrainModel()
    train.load_data('/Users/long/Documents/BCI/python_scripts/grasp/TSceptionEEG/data/data_split.hdf')
