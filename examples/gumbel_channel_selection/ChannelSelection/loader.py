...
def all_subject_loader_HGD(batch_size,train_split,path,num_subjects):
	...
	for k in range(num_subjects):
		...
		train_eeg_data = torch.Tensor(np.load(traindatapath)) # torch.Size([319, 44, 1125])
		train_labels = torch.LongTensor(np.load(trainlabelpath)) # torch.Size([319])
