...
class MLEngine:
    def __init__(self,X_train,y_train,data_path='',file_to_load='',subject_id='',sessions=[1, 2],ntimes=1,kfold=2,m_filters=2,window_details={}):
    ...
    def experiment(self):
        ...
        '''for BCIC Dataset'''
        # dict_keys(['x_data', 'y_labels', 'fs']):  (288, 22, 2376), (288,), 250.0
        #bcic_data = LoadData.LoadBCIC(self.file_to_load, self.data_path)
        #eeg_data = bcic_data.get_epochs()

        eeg_data={}
        eeg_data['x_data']=self.X_train
        eeg_data['y_labels']=self.y_train
        eeg_data['fs']=500
...
class FilterBank:
    def __init__(self,fs):
        ...
        self.gstop = 30
        self.filter_coeff={}
        self.mybands = np.array([[1,4],[4,8],[8,13],[13,30],[60,75],[75,95],[105,125],[125,145],[155,195]])
    def get_filter_coeff(self):
        Nyquist_freq = self.fs/2
        bandwidth=[tmp[1]-tmp[0] for tmp in self.mybands]
        for i, band in enumerate(self.mybands):
            f_pass = np.asarray([self.mybands[i][0], self.mybands[i][0]+bandwidth[i]])
            if i ==0:
    ...
    def filter_data(self,eeg_data,window_details={}):
        ...
        for i, fb in self.filter_coeff.items():
            b = fb.get('b')
            a = fb.get('a')
            eeg_data_filtered = np.asarray([signal.lfilter(b,a,eeg_data[j,:,:]) for j in range(n_trials)]) # (70, 6, 2000)
            if window_details:
