...
from grasp.raw3DToRealForce.utils import evaluate, train, dataset
from grasp.utils import read_fbanddata,plotloss,read_rawdata,read_rawdata_3D


basedir='/Users/long/BCI/python_scripts/grasp/raw3DToRealForce/'
resultdir=basedir+'result/'

#99 train trails, 5 test trials
#trainx, trainy, testx, testy =read_rawdata_3D() # trainx: (114, 33293)
...
trainx=np.load(basedir+'trainx.npy') #(19, 15000, 133)
trainy=np.load(basedir+'trainy.npy') #(15000, 133)
testx=np.load(basedir+'testx.npy') #(19, 15000, 8)
testy=np.load(basedir+'testy.npy') #(15000, 8)

trainx=trainx.swapaxes(1,2)
trainx=trainx.swapaxes(0,1) #(133, 19, 15000)
trainy=trainy.swapaxes(0,1) #(133, 15000)
testx=testx.swapaxes(1,2)
testx=testx.swapaxes(0,1) #(8, 19, 15000)
testy=testy.swapaxes(0,1) #(8, 15000)
