...
Status:
* These models.bak are a work in progress, experiments ongoing.
* Pretrained weights for two models.bak so far, more to come.
* Model details updated to closer match official JAX code now that it's released
* NF-ResNet, NF-RegNet-B, and NFNet-F models.bak supported
...
class NfCfg:
    ...
    extra_conv: bool = False  # extra 3x3 bottleneck convolution for NFNet models.bak
...
model_cfgs = dict(
    # NFNet-F models.bak w/ GELU compatible with DeepMind weights
    ...
    # NFNet-F models.bak w/ GELU (I will likely deprecate/remove these models.bak and just keep dm_ ver for GELU)
    ...
    # NFNet-F models.bak w/ SiLU (much faster in PyTorch)
...
def create_stem(in_chs, out_chs, stem_type='', conv_layer=None, act_layer=None, preact_feature=True):
    ...
    if 'deep' in stem_type:
        if 'quad' in stem_type:
            # 4 deep conv stack as in NFNet-F models.bak
...
class NormFreeNet(nn.Module):
    ...
    This model aims to cover both the NFRegNet-Bx models.bak as detailed in the paper's code snippets and
    the (preact) ResNet models.bak described earlier in the paper.

    There are a few differences:
        * channels are rounded to be divisible by 8 by default (keep tensor core kernels happy),
            this changes channel dim and param counts slightly from the paper models.bak
    ...
    def __init__(self, cfg: NfCfg, num_classes=1000, in_chans=3, global_pool='avg', output_stride=32,
        ...
        if cfg.num_features:
            # The paper NFRegNet models.bak have an EfficientNet-like final head convolution.
