...
class DA_RNN(nn.Module):
    ...
    def __init__(self, X, y, T,
        ...
        # Training set
        self.train_timesteps = int(self.X.shape[0] * 0.3)
    ...
    def trainAndTest(self):
        ...
        for epoch in range(self.epochs):
            ...
            if epoch % 5 == 0:
                y_test_pred = self.test()
                fig, ax = plt.subplots(figsize=(6, 3))
                plt.ion()
                ax.clear()
                #ax.plot(range(1, 1 + len(self.y)), self.y, label="True")
                ax.plot(self.y[self.train_timesteps:self.X.shape[0]], label="True", linewidth=0.1)
                #ax.plot(range(self.T, len(y_train_pred) + self.T),y_train_pred, label='Predicted - Train')
                ax.plot(y_test_pred, label='Predicted - Test',linewidth=0.1)
                ax.legend(loc='upper left')
                figname = '/Users/long/BCI/python_scripts/grasp/NarxModel/result/prediction'+str(epoch)+'.pdf'
                fig.savefig(figname)
                plt.close(fig)
