...
meg_epochs = Epochs(raw, events, tmin=0., tmax=1.5, baseline=None,detrend=1, decim=12)
emg_epochs = Epochs(emg, events, tmin=0., tmax=1.5, baseline=None)

# Prepare classification
X = meg_epochs.get_data() # (199, 183, 151)
y = emg_epochs.get_data().var(axis=2)[:, 0]  # target is EMG power (199,)

# Classification pipeline with SPoC spatial filtering and Ridge Regression
spoc = SPoC(n_components=2, log=True, reg='oas', rank='full')

a=spoc.fit_transform(X,y) # (199, 2)
