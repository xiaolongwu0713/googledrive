...
default_cfgs = {
    # patch models.bak (weights from official Google JAX impl)
    ...
    # patch models.bak, imagenet21k (weights from official Google JAX impl)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        Args:
            ...
            distilled (bool): model includes a distillation token and head as in DeiT models.bak
        ...
        self.num_features = self.embed_dim = embed_dim  # num_features for consistency with other models.bak
...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
            # For old models.bak that I trained prior to conv based patchification
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.bak.')

    # NOTE this extra code to support handling of repr size for in21k pretrained models.bak
